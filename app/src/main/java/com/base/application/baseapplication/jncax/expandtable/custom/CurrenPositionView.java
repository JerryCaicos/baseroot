package com.base.application.baseapplication.jncax.expandtable.custom;
...
public class CurrenPositionView extends LinearLayout
{
	...
	private void initView()
	{
		...
		View rootView = layoutInflater.inflate(R.layout.acjn_pulldown_view,this);
		...
		for(int n = 1;n <= h;n++)
		{
			...
			for(int m = 0;m < length;m++)
			{
				...
				mview.setOnClickListener(((ExpandTableActivity)getContext()).new OnItemClick(data,
						location,n,tableLayout,mview));
				row.addView(mview);
				location++;
			}
			// 每一列 tablerow 增加一个 下拉的layout
			View view = layoutInflater.inflate(R.layout.acjn_expand_item,null);
			view.setTag(n);
