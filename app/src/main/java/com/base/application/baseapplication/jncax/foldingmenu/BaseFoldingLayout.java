...
/**
 * Created by JerryCaicos on 16/1/22 10:30.
 */
...
public class BaseFoldingLayout extends ViewGroup
{
	...
	private void prepareFold(Orientation orientation,float anchorFactor,int numberOfFolds)
	{
		...
		int delta = Math.round(
				mIsHorizontal ? ((float)w)/ ((float)mNumberOfFolds) : ((float)h)/ ((float)mNumberOfFolds));
	...
	private void calculateMatrices()
	{
		 ...
		 * theorem.
		 */
		float depth = mIsHorizontal ? (float)Math.sqrt((double)(mFoldDrawWidth * mFoldDrawWidth - translatedDistanceFoldSquared))
									: (float)Math.sqrt((double)(mFoldDrawHeight * mFoldDrawHeight - translatedDistanceFoldSquared));
		...
		for(int x = 0;x < mNumberOfFolds;x++)
		{
			...
			if(mIsHorizontal)
			{
				mDst[0] = (anchorPoint > x * mFoldDrawWidth)
				...
				mDst[4] = (anchorPoint > (x + 1) * mFoldDrawWidth)
